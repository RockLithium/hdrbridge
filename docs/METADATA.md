...
Inspector data is read from the source container. The canonical HDR working
raster is never substituted for the encoded source representation.
...
Exif, XMP, ICC and Orientation report `Present`, `Absent`, `Unsupported` or
`Read error`. JPEG range is `Unknown` when there is no authoritative signal.
...
| Direct HDR AVIF | normalized Exif/XMP items plus target NCLX | conflicting source ICC/color signaling is not copied |
| Direct HDR TIFF | XMP, target ICC, Orientation 1 and Software tag | arbitrary source Exif IFD cloning is not implemented |
| FP16/RGB10 JPEG XR | WIC pixel format and codec properties | current WIC encoder does not clone Exif/XMP/ICC payloads |

Apple, Adobe and Ultra HDR relationship metadata is input reconstruction data.
It is removed after reconstruction when it would misdescribe a new direct-HDR
output. Camera Exif is retained where the destination encoder supports it.
