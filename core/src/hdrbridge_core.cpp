...
void encode_tiff(const std::filesystem::path& path, const DecodedImage& decoded,
  ...
  TIFFSetField(tiff.get(), TIFFTAG_ZIPQUALITY, std::clamp(options.tiff_compression_level, 1, 9));
#endif
  TIFFSetField(tiff.get(), TIFFTAG_ROWSPERSTRIP,
               TIFFDefaultStripSize(tiff.get(), decoded.info.width * 3u * sizeof(uint16_t)));
  ...
  TIFFSetField(tiff.get(), TIFFTAG_SOFTWARE, "HDR Bridge 0.2");
...
Verification decode_tiff(const std::filesystem::path& path,
  ...
  const bool rec2020 = description.find("Rec.2020 PQ") != std::string::npos;
  const bool p3 = description.find("P3 PQ") != std::string::npos;
  ...
  v.checks.push_back(rec2020 || p3 ? "generated PQ ICC profile identifies direct HDR" : "direct-HDR PQ ICC missing");
  if (expected) v.checks.push_back(v.exact_roundtrip ? "exact RGB16 pixel roundtrip" : "RGB16 pixel roundtrip mismatch");
